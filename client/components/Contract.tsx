...
const STATUS_CONFIG: Record<string, { color: string; bg: string; border: string; dot: string; variant: "success" | "warning" | "info" }> = {
  ...
  InTransit: { color: "text-[#a78bfa]", bg: "bg-[#a78bfa]/10", border: "border-[#a78bfa]/20", dot: "bg-[#a78bfa]", variant: "info" },
};

// ── Main Component ───────────────────────────────────────────

type Tab = "track" | "add" | "update" | "transfer";
...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  const [addStatus, setAddStatus] = useState("Created");
  ...
  const handleCreateProduct = useCallback(async () => {
    ...
    if (!addId.trim() || !addOrigin.trim() || !addStatus.trim()) return setError("Fill in all fields");
    setError(null);
    setIsAdding(true);
    setTxStatus("Awaiting signature...");
    try {
      await createProduct(walletAddress, addId.trim(), addOrigin.trim(), addStatus.trim());
      setTxStatus("Product registered on-chain!");
      setAddId("");
      setAddOrigin("");
      setAddStatus("Created");
  ...
  }, [walletAddress, addId, addOrigin, addStatus]);
  ...
  const handleTransferOwnership = useCallback(async () => {
    if (!walletAddress) return setError("Connect wallet first");
    if (!transferId.trim()) return setError("Enter a product ID");
    setError(null);
    setIsTransferring(true);
    setTxStatus("Awaiting signature...");
    try {
      await transferOwnership(walletAddress, transferId.trim());
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode; color: string }[] = [
    ...
    { key: "transfer", label: "Transfer", icon: <TransferIcon />, color: "#a78bfa" },
  ];
      ...
      {txStatus && (
        <div className="mb-4 flex items-center gap-3 rounded-xl border border-[#34d399]/15 bg-[#34d399]/[0.05] px-4 py-3 backdrop-blur-sm shadow-[0_0_30px_rgba(52,211,153,0.05)] animate-slide-down">
          <span className="text-[#34d399]">
            {txStatus.includes("on-chain") || txStatus.includes("updated") || txStatus.includes("transferred") ? <CheckIcon /> : <SpinnerIcon />}
          </span>
      ...
      <Spotlight className="rounded-2xl">
        <AnimatedCard className="p-0" containerClassName="rounded-2xl">
          ...
          <div className="p-6">
                  ...
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden animate-fade-in-up">
                    <div className="border-b border-white/[0.06] px-4 py-3 flex items-center justify-between">
                      ...
                      {(() => {
                        ...
                        const cfg = STATUS_CONFIG[status.replace(/\s/g, "")];
                        return cfg ? (
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {Object.entries(productData).map(([key, val]) => (
                        <div key={key} className="flex items-center justify-between">
                          ...
                          <span className="font-mono text-sm text-white/80 break-all max-w-[280px] text-right">{val}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {activeTab === "add" && (
              <div className="space-y-5">
                <MethodSignature name="create_product" params="(id, origin, status, owner)" color="#7c6cf0" />
                ...
                <Input label="Initial Status" value={addStatus} onChange={(e) => setAddStatus(e.target.value)} placeholder="e.g. Created" />
                {walletAddress ? (
                  <ShimmerButton onClick={handleCreateProduct} disabled={isAdding} shimmerColor="#7c6cf0" className="w-full">
            ...
            {activeTab === "update" && (
              <div className="space-y-5">
                <MethodSignature name="update_status" params="(caller, id, new_status)" color="#fbbf24" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(["Shipped", "In Transit", "Delivered"] as const).map((s) => {
                      const cfg = STATUS_CONFIG[s.replace(/\s/g, "")];
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            active
                              ? `${cfg?.border ?? "border-white/[0.1]"} ${cfg?.bg ?? "bg-white/[0.05]"} ${cfg?.color ?? "text-white/70"}`
                              : "border-white/[0.06] bg-white/[0.02] text-white/35 hover:text-white/55 hover:border-white/[0.1]"
                          )}
                        >
                          <span className={cn("h-1.5 w-1.5 rounded-full transition-colors", active ? cfg?.dot ?? "bg-white/40" : "bg-white/20")} />
                          {s}
                        </button>
                      );
                    })}
            ...
            {activeTab === "transfer" && (
              <div className="space-y-5">
                <MethodSignature name="transfer_ownership" params="(caller, id)" color="#a78bfa" />
                <Input label="Product ID" value={transferId} onChange={(e) => setTransferId(e.target.value)} placeholder="e.g. PROD-001" />
                <p className="text-xs text-white/25">
                  This transfers ownership to your connected wallet address. Anyone can call this (permissionless).
                </p>
                {walletAddress ? (
                  <ShimmerButton onClick={handleTransferOwnership} disabled={isTransferring} shimmerColor="#a78bfa" className="w-full">
                    {isTransferring ? <><SpinnerIcon /> Transferring...</> : <><TransferIcon /> Take Ownership</>}
                  </ShimmerButton>
                ) : (
                  <button
                    onClick={onConnect}
          ...
          <div className="border-t border-white/[0.04] px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {["Created", "Shipped", "In Transit", "Delivered"].map((s, i) => (
                <span key={s} className="flex items-center gap-1.5">
                  <span className={cn("h-1 w-1 rounded-full", STATUS_CONFIG[s.replace(/\s/g, "")]?.dot ?? "bg-white/20")} />
